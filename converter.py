...
class Converter:
    ...
    def O3DPointCloud_to_nparray(self,open3d_cloud,min_intensity=-1,max_intensity=-1):
        ...
        if len(colors)==0 or min_intensity!=-1 or max_intensity!=-1:
            ...
            return np_open3d_cloud_coordinates.reshape(-1,3),[]


    def nparray_to_ROSpc2(self,np_open3d_cloud,frame_id="map",np_open3d_cloud_intensities=[]):
    ...
    def O3DPointCloud_to_ROSpc2(self,open3d_cloud, frame_id="map",min_intensity=-1,max_intensity=-1):
        ...
        if max_intensity==-1:
            ...
            self.max_intensity=max_intensity
        np_open3d_cloud_coordinates,np_open3d_cloud_intensities = self.O3DPointCloud_to_nparray(open3d_cloud,min_intensity,max_intensity)
        return self.nparray_to_ROSpc2(np_open3d_cloud_coordinates, frame_id,np_open3d_cloud_intensities)
    ...
    def ROSpc2_to_nparray(self,pc2):
        ...
        if intensity_index != -1:#Reference: abhishekbawkar's comment on https://github.com/isl-org/Open3D/issues/921
            ...
            intensities_norm = self.normalize_np(intensity,0,65535)
    ...
    def nparray_to_O3DPointCloud(self, xyz, colors=-1):
    ...
    def ROSpc2_to_O3DPointCloud(self,pc2):
        ...
        xyz,colors,min_intensity,max_intensity = self.ROSpc2_to_nparray(pc2)
        return self.nparray_to_O3DPointCloud(xyz,colors),min_intensity,max_intensity#3 cause xyz coordinates and intensity is 1